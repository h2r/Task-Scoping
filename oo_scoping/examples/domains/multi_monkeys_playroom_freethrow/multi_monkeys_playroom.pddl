...
(:action throw_ball_miss_bell
    :parameters (?ba - ball ?e - eye ?ha - hand ?ma - marker)
    :precondition (and
                        ;eyes, hands ready to throw
                       (= (eye-x ?e) (ball-x ?ba)) 
                       (= (eye-y ?e) (ball-y ?ba))
                       (= (hand-x ?ha) (ball-x ?ba))
                       (= (hand-y ?ha) (ball-y ?ba))
                       ; No bell will be hit while monkey is paying attention
                       (or 
                            ; No bell hit
                            (not 
                            ;We use nested quantifiers because our parser can't handle it otherwise (todo fix that)
                                    ( exists (?be - bell )
                                        ( exists (?mo - monkey)
                                            (and 
                            ...
                            ; Monkey not paying attention
                            ( not (forall (?gb - greenbutton) (gbutton-on ?gb)))
                       )
    )
