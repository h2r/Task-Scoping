...
then
    ...
    until [ $COUNTER -lt 1 ]; do
        ...
        exec 3>&1 4>&2
        foo=$( { time python oo_scoping/scope_and_writeback_pddl.py --domain oo_scoping/examples/domains/minecraft3/minecraft-contrived3.pddl --prob oo_scoping/examples/domains/minecraft3/prob_get_dyed_wool_irrel.pddl 1>&3 2>&4; } 2>&1 )  # Captures time only.
        exec 3>&- 4>&-
        sruntimes+=("$foo")
        let COUNTER-=1
    done
...
until [ $COUNTER -lt 1 ]; do
    echo RUN_NUMBER $COUNTER
    exec 3>&1 4>&2
    foo=$( { time enhsp-2020 -o oo_scoping/examples/domains/minecraft3/minecraft-contrived3.pddl -f oo_scoping/examples/domains/minecraft3/prob_get_dyed_wool_irrel.pddl -planner opt-hmax 1>&3 2>&4; } 2>&1 )  # Captures time only.
    exec 3>&- 4>&-
    enhsp_runtimes+=("$foo")
    let COUNTER-=1
done
...
until [ $COUNTER -lt 1 ]; do
    echo RUN_NUMBER $COUNTER
    exec 3>&1 4>&2
    foo=$( { time enhsp-2020 -o oo_scoping/examples/domains/minecraft3/minecraft-contrived3_scoped_prob_get_dyed_wool_irrel.pddl -f oo_scoping/examples/domains/minecraft3/prob_get_dyed_wool_irrel_scoped.pddl -planner opt-hmax 1>&3 2>&4; } 2>&1 )  # Captures time only.
    exec 3>&- 4>&-
    senhsp_runtimes+=("$foo")
    let COUNTER-=1
done
