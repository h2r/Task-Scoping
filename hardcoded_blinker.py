...
def make_movement_skills(passengers: List[Passenger], taxi: Taxi, action_name, var_affected, blinker=False,
						 blinker_affects=None, effect_index = 0):
	skills = []
	# 	Empty Taxi
	...
	if blinker:
		...
		effect_b_off = effect
		effect_b_on = effect + [EffectTypePDDL(getattr(taxi, blinker_affects),effect_index)]
	...
	for i, p in enumerate(passengers):
		...
		cond = z3.And(*conds)
		effect = [EffectTypePDDL(getattr(taxi, var_affected),0), EffectTypePDDL(getattr(p, var_affected),effect_index)]
		if blinker:
			cond_b_off = z3.And(cond, z3.Not(taxi.blinker))
			cond_b_on = z3.And(cond, taxi.blinker)
			effect_b_off = effect
			effect_b_on = effect + [EffectTypePDDL(getattr(taxi, blinker_affects),effect_index), EffectTypePDDL(getattr(p, blinker_affects),effect_index)]
			skills.append(SkillPDDL(cond_b_off, action_name, effect_b_off))
			skills.append(SkillPDDL(cond_b_on, action_name, effect_b_on))
		else:
			skills.append(SkillPDDL(cond, action_name, effect))
...
def make_pickup_dropoff_skills(ps: List[Passenger], t: Taxi):
	...
	for p_id, p in enumerate(ps):
		...
		dropoff_skill = SkillPDDL(p.intaxi, "dropoff()", EffectTypePDDL(p.intaxi, False))
...
def prepare_taxi_domain(n_passegners=2, blinker = False, goal = (6, 8)):
	...
	taxi = make_taxis(1)[0]
	move_north = make_movement_skills(passengers, taxi, "move_north()", 'y', blinker, 'x', effect_index=1)
	move_south = make_movement_skills(passengers, taxi, "move_south()", 'y', False, 'x', effect_index=-1)
	move_east = make_movement_skills(passengers, taxi, "move_east()", 'x', False, 'y', effect_index=1)
	move_west = make_movement_skills(passengers, taxi, "move_west()", 'x', False, 'y', effect_index=-1)
