...
until [ $COUNTER -lt 1 ]; do
    echo RUN_NUMBER $COUNTER
    exec 3>&1 4>&2
    foo=$( { time python $2 --alias seq-opt-lmcut /home/<redacted>/Documents/planutils_stuff/OO-Scoping-IPC/logistics20.sas 1>&3 2>&4; } 2>&1 )  # Captures time only.
    exec 3>&- 4>&-
    fd_runtimes+=("$foo")
    let COUNTER-=1
done
...
until [ $COUNTER -lt 1 ]; do
    echo RUN_NUMBER $COUNTER
    exec 3>&1 4>&2
    foo=$( { time python $2 --alias seq-opt-lmcut /home/<redacted>/Documents/planutils_stuff/OO-Scoping-IPC/logistics20_scoped.sas 1>&3 2>&4; } 2>&1 )  # Captures time only.
    exec 3>&- 4>&-
    sfd_runtimes+=("$foo")
    let COUNTER-=1
done
